...
import { X, LoaderCircle, Copy, ArrowRight } from "lucide-react";
...
const InterviewModal = ({ isOpen, onClose, resumeId }: InterviewModalProps) => {
  // Setup state
  ...
  const handleStart = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/ai/interview/generate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ resumeId, count, types, focus, jobDescription })
      });
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/ai/interview/evaluate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ resumeId, question: currentQ.question, answer: userAnswer })
      });
  ...
  // Generate summary
  const generateSummary = (allAnswers: Answer[]) => {
    const total = allAnswers.length;
    const ratings = allAnswers.map(a => a.evaluation.rating);
    // Simple mapping: Excellent=5, Good=4, Needs Improvement=3, etc.
    const scoreMap: Record<string, number> = { "Excellent": 5, "Good": 4, "Needs Improvement": 3 };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6 relative">
        ...
        <div className="flex justify-end">
          ...
          >
            {evaluating ? <LoaderCircle className="w-4 h-4 animate-spin" /> : "📤 Submit Answer"}
          </button>
        </div>
      </div>
    </div>
